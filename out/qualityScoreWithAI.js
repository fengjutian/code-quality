...
exports.QualityScorerAI = void 0;
// src/qualityScoreWithAI.ts
const eslint_1 = require("eslint");
const fs = require("fs");
const openai_1 = require("openai"); // 使用官方 openai SDK
class QualityScorerAI {
    ...
    calculateTotalScore(results) {
        ...
        const totalScore = results.reduce((sum, r) => {
            const weighted = r.eslintScore * this.options.eslintWeight +
    ...
    /** ESLint 分析 + 提取问题 */
    async calculateESLintScore(filePath) {
        const eslint = new eslint_1.ESLint({ fix: false });
    ...
    /** 启发式评分 */
    calculateHeuristicScore(codeText) {
    ...
    /** 结构惩罚 */
    calculateStructurePenalty(codeText) {
    ...
    getMaxNestingDepth(codeText) {
        let maxDepth = 0;
        let currentDepth = 0;
        const lines = codeText.split(/\r?\n/);
        for (const line of lines) {
            if (line.match(/\b(if|for|while|switch|try)\b/))
                currentDepth++;
            if (line.match(/\}/))
                currentDepth--;
    ...
    /** 使用大模型生成优化建议 */
    async generateAISuggestions(issues, codeText) {
        if (issues.length === 0)
            return [];
        const prompt = `
...
exports.QualityScorerAI = QualityScorerAI;
// 使用示例
async function main() {
...
// main();
//# sourceMappingURL=qualityScoreWithAI.js.map