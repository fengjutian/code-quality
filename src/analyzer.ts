...
// 评估函数长度
function evaluateFunctionLength(code: string): { score: number; issues: string[] } {
    const issues: string[] = [];
...
// 评估嵌套深度
function evaluateNestedDepth(code: string): { score: number; issues: string[] } {
    const issues: string[] = [];
    let maxDepth = 0;
    let currentDepth = 0;
...
// 检测代码重复度
function detectCodeDuplication(code: string): { score: number; issues: string[] } {
    const issues: string[] = [];
    const lines = code.split('\n');
...
// 评估注释密度
function evaluateCommentDensity(code: string): { score: number; issues: string[] } {
    const issues: string[] = [];
    const lines = code.split('\n');
...
// 检查变量命名规范
function checkVariableNaming(code: string): { score: number; issues: string[] } {
    const issues: string[] = [];
...
// 检测魔法数字
function detectMagicNumbers(code: string): { score: number; issues: string[] } {
    const issues: string[] = [];
...
// 检查函数参数数量
function checkFunctionParameters(code: string): { score: number; issues: string[] } {
    const issues: string[] = [];
