...
export function generateAIReportHTML(assessment: CodeQualityAssessment): string {
    // 根据分数获取对应的颜色
    const getScoreColor = (score: number): string => {
        if (score >= 9) return '#27ae60'; // 优秀
        if (score >= 7) return '#2ecc71'; // 良好
        if (score >= 5) return '#f39c12'; // 中等
        if (score >= 3) return '#e67e22'; // 较差
        return '#e74c3c'; // 很差
    };
